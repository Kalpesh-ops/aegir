...
export default function ScanPage() {
    ...
    pollIntervalRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (res.status === 'running') {
          ...
          const aiSummary = res.result_json?.ai_summary || 'No report generated.'
          const firewall = res.result_json?.firewall || null
          const traffic = res.result_json?.traffic || null
          setResults({ ai_summary: aiSummary, ports: portData, cve_findings: finalCves, firewall, traffic })
          if (firewall) addLog('info', 'FW', `Firewall: ${firewall.firewall_status}`)
          if (traffic?.status === 'captured') addLog('info', 'CAP', `Traffic: ${traffic.total_packets || '?'} packets captured`)
  ...
  function downloadJSON() {
    ...
    const blob = new Blob([JSON.stringify({
      ...
      firewall: results.firewall || null,
      traffic: results.traffic || null,
      scan_mode: scanMode,
