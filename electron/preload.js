 ...
 *
 * Exposes a minimal, typed API on `window.netsec`. Everything else the
...
contextBridge.exposeInMainWorld('netsec', {
  /** Returns the local backend URL, e.g. "http://127.0.0.1:58123". */
  getBackendUrl: () => ipcRenderer.invoke('netsec:getBackendUrl'),

  /** Returns build/runtime metadata for the About dialog + crash reports. */
  getAppInfo: () => ipcRenderer.invoke('netsec:getAppInfo'),
});
